...
Cette documentation se focalise uniquement sur les aspects du paramétrage d'un PRESET, et rend d'autre.
...
En appyant sur les touches `+` et `-` vous pouvez faire défiler les options suivantes :
...
La position d'un oscillateur (A, B, C, ou D) peut-être réglée à l'aide du joystick et avoir une valeur entre 0 et 99, déterminant à quel point chacun d'eux dominent à cette étape.
...
Ainsi, à l'aide du **joystick**, il est possible de **doser la dominance** de chaque oscillateur à chaque étape.
...
Sur le Pro VS Mini, vous avez **trois enveloppes indépendantes**, accessibles via le bouton ENVELOPE :
...
> Ces durées sont exprimées en valeurs de 0 à 99. Plus la valeur est élevée, plus la transition entre les deux points est lente (le morphing dure plus longtemps).

## D. Les formes d'onde

Le Pro VS Mini dispose de 128 formes d'onde, que l'on peut régler pour chacun des  4 oscillateurs A, B, C et D via le bouton `OSC` pour choisir quel oscillateur et les potentiomètres `WAVE` et `FREQ`, qui respectivement permettent de changer la forme d'onde et de faire varier la fréquence (si nécessaire) pour chacun d'eux.

Notons qu'il est possible rendre un oscillateur silencieux à l'aide du numéro 126 et qu'il est permis d'exploiter un bruit blanc à l'aide du numéro 127.

Voici la liste des formes d'onde :
